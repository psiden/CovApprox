...
//    mexErrMsgTxt("Third argument should be a sparse, double matrix.");
  ...
  size_t BI = (size_t) mxGetScalar(prhs[3]);
//   mexPrintf("%i\n",BI);
  if(BI < 1)
    mexErrMsgTxt("BI must be a positive integer");
  int Nthreads=8;
  if(nrhs==5){
    if(!mxIsRealPosScalar(prhs[4]))
      mexErrMsgTxt("Fifth argument should be a real scalar.");
    double *ptrData = mxGetPr(prhs[4]);
    if(ptrData==NULL)
      mexErrMsgTxt("Unable to obtain pointers to fifth argument.");
  ...
  if(n < BI)
    mexErrMsgTxt("BI must be smaller than matrix size");
